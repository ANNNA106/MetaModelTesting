...
def fetch_dsa(model, x_train, x_target, target_name, layer_names, args):
    ...
    dsa_file = os.path.join(args.save_path, target_name + args.d + '_dsaats.npy')
    if os.path.exists(dsa_file):
        print("Found saved {} DSAs, skip serving".format(target_name))
        # In case target_ats is stored in a disk
        dsa = np.load(dsa_file)
        return list(dsa)
...
def fetch_lsa(model, x_train, x_target, target_name, layer_names, args):
    ...
    lsa_file = os.path.join(args.save_path, target_name + args.d + '_lsa.npy')
    if os.path.exists(lsa_file):
        print("Found saved {} LSAs, skip serving".format(target_name))
        lsa = np.load(lsa_file)
        return list(lsa)
